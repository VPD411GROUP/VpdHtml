...
document.addEventListener("keydown", (event) => {
  ...
  if (key === "Enter" || key === "=") {
    ...
    addToHistory(`${previousNumber} ${operator} ${currentNumber} = ${result}`)
...
// ===============
// 10. Добавление в историю
// ===============

const historyList = document.getElementById("history-list")

// entry - передаваемый результат вычисления
function addToHistory(entry){
    // создаём элемент li и получаем его в переменной
    const li = document.createElement("li")
    
    // ставим текст li на entry
    li.textContent = entry

    // с помощью prepend добавляем в начало
